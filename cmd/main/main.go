...
func handler() error {
	...
	for r := range checks {
		fmt.Println(fmt.Sprintf("Checked %s price is %.2f change is %t", r.URL, r.CurrentPrice, r.Changed))
		if r.Error != nil {
			fmt.Println(fmt.Sprintf("Error %s", r.Error))
			err = email.SendError(config.MailNotifyAddress, r.URL, r.Error)
		}
		if r.Changed {
			fmt.Println(fmt.Sprintf("Price changed %.2f", r.CurrentPrice))
			err = email.SendPriceChange(config.MailNotifyAddress, r.URL, r.CurrentPrice, r.LastPrice)
		}
		if err != nil {
			return err
		}
